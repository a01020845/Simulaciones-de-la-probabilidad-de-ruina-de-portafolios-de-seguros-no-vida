...
u <- 32 # capital inicial
c <- 3 # tasa de ingreso de primas por unidad de tiempo
...
for (i in 1:nsim) {
  ...
  siniestros <- rlnorm(n_sin, meanlog = mu, sdlog = sigma)
...
z <- 1.96  # valor z para 95 por ciento
se <- sqrt(prob_ruina * (1 - prob_ruina) / nsim)  # error estandar
ic_inf <- prob_ruina - z * se
ic_sup <- prob_ruina + z * se
# Nota: 90 por ciento z = 1.645, 95 por ciento z = 1.96, 99 por ciento z = 2.576

### Resumen ###

## Cuadro Resumen
cat("Probabilidad estimada de ruina:", round(prob_ruina, 4), "\n")
cat("Intervalo de confianza al 95 por ciento: [", round(ic_inf, 4), ",", round(ic_sup, 4), "]\n")
