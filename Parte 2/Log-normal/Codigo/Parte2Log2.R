...
# Log-normal (Proceso de Renovacion)
...
simular_resultado <- function(u, c, Tmax = 120, nsim = 250,
  ...
  for (i in 1:nsim) {
    ...
    arribos_brutos_sin <- cumsum(rlnorm(n = Tmax * 2 / exp(mu + 0.5 * sigma^2), 
    ...
    inyecciones <- rgamma(n_iny, shape = alpha, scale = sigma_2)
...
write.csv(rejilla, file = "C:/Users/DiDi/Documents/JIM/Tesis/Simulaciones/Parte 2/Log-normal/Outputs/P2_Rej_Log.csv", row.names = FALSE)
