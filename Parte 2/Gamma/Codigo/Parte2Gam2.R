...
simular_resultado <- function(u, c, Tmax = 120, nsim = 250,
  ...
  for (i in 1:nsim) {
    ...
    arribos_brutos_sin <- cumsum(rgamma(n = Tmax * 2 / (alpha * sigma), 
                                        shape = alpha, scale = sigma))
    ...
    arribos_brutos_iny <- cumsum(rexp(n = Tmax * lambda * 2, rate = lambda))
    ...
    inyecciones <- rgamma(n_iny, shape = alpha_2, scale = sigma_2)
...
ggplot(factibles, aes(x = u, y = c, fill = utilidad_prom)) +
  ...
  labs(title = "Combinaciones viables (ruina <= 1%, utilidad >= 18)",
       x = "u (capital inicial)", y = "c (tasa de primas)")

# Exportar la tabla rejilla a CSV
write.csv(rejilla, file = "C:/Users/DiDi/Documents/JIM/Tesis/Simulaciones/Parte 2/Gamma/Outputs/P2_Rej_Gam.csv", row.names = FALSE)
